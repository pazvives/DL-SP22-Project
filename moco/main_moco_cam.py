...
class UnlabeledDataset(torch.utils.data.Dataset):
    def __init__(self, root, box_path, transform):
        """
        Args:
        ...
        self.image_indexes = [int(n.split(".")[0]) for n in image_names] #Assuming names are "<number>.<extension>"
        self.num_images = len(self.image_indexes)
        self.offset = 0
        self.boxes = load_boxes(box_path)
    ...
    def __getitem__(self, idx):
        ...
        actual_idx = self.image_indexes[idx]
        with open(os.path.join(self.image_dir, f"{actual_idx+self.offset}.PNG"), 'rb') as f:
            img = Image.open(f).convert('RGB')

        boxes = self.boxes[str(actual_idx)]
        if len(boxes) > 0:
            box = random.choice(boxes)
            box = upscale_box(box, img.size)
            img = img.crop(box)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    train_dataset = UnlabeledDataset(root=os.path.join(args.data, 'train/unlabeled'), 
                                     box_path='save_box/box_10.txt',
                                     transform=moco.loader.TwoCropsTransform(transforms.Compose(augmentation)))

    print("The length of pre-training set is: " + str(len(train_dataset)))
